...
class ViewController: UIViewController {
    ...
    lazy fileprivate var webView: WKWebView = {
        ...
        let script = WKUserScript(source: "changeHeaderColor()", injectionTime: .atDocumentEnd, forMainFrameOnly: true)
         let script2 = WKUserScript(source: "changeHeaderColors()", injectionTime: .atDocumentEnd, forMainFrameOnly: true)
    ...
    @IBAction func btnTest(_ sender: UIButton) {
        webView.evaluateJavaScript("changeHeaderColor2()") { (a, b) in
            print(#function)
        }
    }
}
